...
import io.github.crucible.CrucibleTickManager;
...
public abstract class World implements IBlockAccess
{
   /**
    ...
    public List<Entity> loadedEntityList = new ListSet<>();
    public List<Entity> unloadedEntityList = new ListSet<>(); // Cauldron
    public List<TileEntity> loadedTileEntityList = new TileList(this);
    public List<TileEntity> addedTileEntityList = new TileList(this); // Crucible private > public
    ...
    public CrucibleTickManager myManager = new CrucibleTickManager(this);
    ...
    public void notifyBlocksOfNeighborChange(int p_147459_1_, int p_147459_2_, int p_147459_3_, Block p_147459_4_)
    {
        ...
        this.notifyBlockOfNeighborChange(p_147459_1_, p_147459_2_, p_147459_3_ + 1, p_147459_4_);
        this.spigotConfig.antiXrayInstance.updateNearbyBlocks(this, p_147459_1_, p_147459_2_, p_147459_3_);
    }
    ...
    public void updateEntities()
    {
        ...
        if (!this.field_147483_b.isEmpty())
        {
            ...
            List temporary_tile_entity_list = new TileList(this.loadedTileEntityList.size(),this);
        ...
        if (!(loadedTileEntityList instanceof TileList)) {
            CrucibleModContainer.logger.warn("[Crucible] TileEntity list class is not a TileList ({})! Something changed it and it may cause errors and slow downs with crucible tick skip", loadedTileEntityList);
            loadedTileEntityList = new TileList(loadedTileEntityList, this);
        }
        for (tileLimiter.initTick(); tilesThisCycle < loadedTileEntityList.size() && (tilesThisCycle % 10 == 0 || tileLimiter.shouldContinue()); tileTickPosition++, tilesThisCycle++) {
            ...
            if (tileentity.hasWorldObj() && this.blockExists(tileentity.xCoord, tileentity.yCoord, tileentity.zCoord))
            {
                try
                {
                    if (CrucibleConfigs.configs.crucible_tickHandler_tickSkip && CrucibleTickManager.isLaggy()){
                        if (tileentity.myStopwatch == null) {
                            CrucibleModContainer.logger.warn("[Crucible] Tile '{}' with null stopwatch, this is a bug in crucible!", tileentity.toStringButDifferent());
                            tileentity.myStopwatch = myManager.of(tileentity);
                        }
                        if (tileentity.myStopwatch.canTick()) {
                            tileentity.tickTimer.startTiming(); // Spigot
                            tileentity.myStopwatch.tickStart();
                            tilesTicked++;
                            tileentity.updateEntity();
                            tileentity.myStopwatch.tickStop();
