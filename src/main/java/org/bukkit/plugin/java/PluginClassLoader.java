...
import io.github.crucible.config.DynamicCrucibleConfigs;
...
public class PluginClassLoader extends URLClassLoader {
    ...
    PluginClassLoader(final JavaPluginLoader loader, final ClassLoader parent, final PluginDescriptionFile description, final File dataFolder, final File file) throws InvalidPluginException, MalformedURLException {
        ...
        PluginConfig config = DynamicCrucibleConfigs.configs.getPluginConfig(pluginName);

        // configure default remapper settings
        boolean useCustomClassLoader = config.classloader_custom;
        debug = config.classloader_debug;
        boolean remapNMS1710 = config.classloader_remap_NMS1710;
        boolean remapNMS179 = config.classloader_remap_NMS179;
        boolean remapNMS172 = config.classloader_remap_NMS172;
        boolean remapNMS164 = config.classloader_remap_NMS164;
        boolean remapNMS152 = config.classloader_remap_NMS152;
        String remapNMSPre = config.classloader_remap_NMSPre;
        boolean remapOBC1710 = config.classloader_remap_OBC1710;
        boolean remapOBC179 = config.classloader_remap_OBC179;
        boolean remapOBC172 = config.classloader_remap_OBC172;
        boolean remapOBC164 = config.classloader_remap_OBC164;
        boolean remapOBC152 = config.classloader_remap_OBC152;
        boolean remapOBCPre = config.classloader_remap_OBCPre;
        boolean globalInherit = config.classloader_globalInherit;
        boolean pluginInherit = config.classloader_pluginInherit;
        boolean reflectFields = config.classloader_remap_reflectFields;
        boolean reflectClass = config.classloader_remap_reflectClass;
        boolean allowFuture = config.classloader_remap_allowFuture;
        ...
        if (!remapNMSPre.equals("false") && !remapNMSPre.isEmpty()) {
            switch (remapNMSPre) {
                case "1.7.10":
                    flags |= 0x17100000;
                    break;
        ...
        try {
            ...
            if ((flags & F_REMAP_OBC1710) != 0) {
                if (CauldronUtils.deobfuscatedEnvironment())
                    ...
                    jarMapping.packages.put(org_bukkit_craftbukkit + "/v1_7_R4", org_bukkit_craftbukkit + "/" + getNativeVersion());
            }
            ...
            if ((flags & F_REMAP_OBC179) != 0) {
                if (CauldronUtils.deobfuscatedEnvironment())
                    ...
                    jarMapping.packages.put(org_bukkit_craftbukkit + "/v1_7_R3", org_bukkit_craftbukkit + "/" + getNativeVersion());
            }
